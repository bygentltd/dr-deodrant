      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        {arrangedPosts.map((post, index) => (
          <motion.div
            ...
            transition={{ duration: 0.75, delay: index * 0.12, ease: [0.21, 0.47, 0.32, 0.98] }}
            className={post.featured ? "md:col-span-2" : "col-span-1"}
          >
            <Link
              href={`/blog/${post.slug}`}
              className={`relative group overflow-hidden rounded-2xl md:rounded-3xl block ${
                post.featured ? "h-[280px] sm:h-[400px] md:h-[600px]" : "h-[260px] sm:h-[350px] md:h-[450px]"
              }`}
            >
              <img
                src={post.heroImage}
