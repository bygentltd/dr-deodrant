    ...
    <section className="py-20 md:py-32 bg-[#F0F7FF] overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-12 lg:gap-8 xl:gap-16 items-center">
          ...
          {/* Left Column (Text Left, Icon Right on large screens) */}
          <div className="flex flex-col gap-10 md:gap-16 order-2 lg:order-1 z-10">
            {leftServices.map((service, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  className="flex flex-col sm:flex-row lg:flex-row items-center lg:justify-end gap-6 text-center sm:text-left lg:text-right reveal-on-scroll"
                  style={{ transitionDelay: `${idx * 100}ms` }}
                >
                  <div className="flex-1 order-2 sm:order-1">
                    <h3 className="text-[#0B2559] font-bold text-lg md:text-xl mb-1.5">{service.title}</h3>
                    <p className="text-gray-500 text-sm leading-relaxed max-w-[240px] mx-auto sm:ml-0 lg:ml-auto">
                      {service.description}
                    </p>
                  </div>
          ...
          <div className="flex justify-center order-1 lg:order-2 reveal-on-scroll z-20">
            <div className="relative w-[300px] h-[480px] md:w-[360px] md:h-[580px] rounded-[200px] overflow-hidden shadow-[0_0_50px_rgba(255,255,255,0.8)] border-[8px] border-white/60">
              {/* Sparkle Decorations (Optional visual flair) */}
          ...
          <div className="flex flex-col gap-10 md:gap-16 order-3 lg:order-3 z-10">
            {rightServices.map((service, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  className="flex flex-col sm:flex-row lg:flex-row items-center lg:justify-start gap-6 text-center sm:text-left reveal-on-scroll"
                  style={{ transitionDelay: `${idx * 100}ms` }}
                >
                  {/* Icon with connecting line */}
                  ...
                  <div className="flex-1">
                    <h3 className="text-[#0B2559] font-bold text-lg md:text-xl mb-1.5">{service.title}</h3>
                    <p className="text-gray-500 text-sm leading-relaxed max-w-[240px] mx-auto sm:ml-0">
                      {service.description}
                    </p>
                  </div>
                </div>
